...
def addcriminalspublic(request):
    #bn=request.session['bnname']
    return render(request,"addcriminalspublic.html")
...
def AddMissingpublic(request):
    #Email= request.session['email']
    #reg=register_form.objects.get(Email=Email)
    #fn=reg.Name
    return render(request,"AddMissingpublic.html")
...
def Missing_details(request):
    ...
    return render(request,'index.html',{'meg':meg,'fn':fn})
def Missing_details_public(request):
    firstname = request.POST.get('firstname')
    lastname = request.POST.get('lastname')
    perno = request.POST.get('perno')
    missedarea = request.POST.get('missedarea')
...
def view_my_complaints(request):
    complaints=complaint.objects.filter(Email=request.session['email'])
    print(complaints)
...
def view_all_complaint(request):
    ...
    return render(request,'view_all_complaint.html')
